...
async def create_job(job: JobCreate, current_user=Depends(get_current_user)):
    ...
    jp = JobPost.objects.create(
        ...
        salary_visibility=job.salary_visibility.value,
    ...
    return {
        ...
        "results": [
            {"id": str(j.id), "title": j.title, "hospital_name": j.hospital.name,
             ...
             "salary_visibility": j.salary_visibility, "published_at": j.published_at.isoformat() if j.published_at else None}
            for j in results
        ],
    }
...
async def get_job(job_id: str, current_user=Depends(get_current_user)):
    from apps.jobs.models import JobPost
    try:
        j = JobPost.objects.select_related('hospital').get(id=job_id, status='PUBLISHED')
...
async def apply_to_job(job_id: str, cv_file_id: Optional[str] = None, current_doctor=Depends(get_current_doctor)):
    ...
    app_obj = JobApplication.objects.create(job=job, doctor=current_doctor, cv_file_id=cv_file_id)
...
async def my_applications(
    ...
    total = qs.count()
    results = qs.order_by('-applied_at')[(page - 1) * page_size: page * page_size]
    return {
...
# ── Hospital side: manage applications ───────────────────────
...
async def update_application_status(
    ...
    try:
        ...
        raise HTTPException(status_code=403, detail="Not associated with a hospital")
    try:
        app_obj = JobApplication.objects.select_related('job').get(id=application_id, job__hospital=hu.hospital)
    except JobApplication.DoesNotExist:
        raise HTTPException(status_code=404, detail="Application not found")
